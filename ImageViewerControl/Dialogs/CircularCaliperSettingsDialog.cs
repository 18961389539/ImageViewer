...
{
    internal sealed class CircularCaliperSettingsDialog : Window
    {
        private readonly CircularCaliperMeasureRoi _model;
        private readonly Action<CircularCaliperMeasureRoi>? _previewAction;
        ...
        public CircularCaliperSettingsDialog(CircularCaliperMeasureRoi model, Action<CircularCaliperMeasureRoi>? previewAction)
        {
            ...
            Title = UiText.Get("CircularCaliperDialogTitle");
            Width = 360;
            Height = 450;
            ...
            {
                Text = UiText.Get("CircularCaliperDialogHeading"),
            ...
            panel.Children.Add(new TextBlock
            {
                Text = UiText.Get("CircularCaliperDialogDescription"),
        ...
        public CircularCaliperMeasureRoi? Result { get; private set; }

        private void OnOkClick(object? sender, RoutedEventArgs e)
        {
            if (!TryApplyInputs(out string errorMessage))
            {
                MessageBox.Show(this, errorMessage, UiText.Get("CircularCaliperDialogTitle"), MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
        ...
        private void PreviewCurrentValues()
        {
            ...
            _previewAction?.Invoke((CircularCaliperMeasureRoi)_model.Clone());
        }
